...
> **TL;DR:** The hand-written API client is the code most likely to lie: it rots the moment the backend changes. Your backend already emits an **OpenAPI** spec, so point [**Orval**](https://orval.dev/) at it and get a fully-typed client, framework-native query hooks, Zod validators, and mock servers, regenerated on every spec change. Stop maintaining the frontend's copy of the backend by hand.
